...
interface LoginFormProps {
    ...
    hideGoogleLogin?: boolean;   // Hides the Google sign-in button
    registerHref?: string;       // If set, "Create one" navigates here instead of calling onSwitchToRegister
}

export const LoginForm = ({ onSwitchToRegister, onClose, role = "SEEKER", hideGoogleLogin = false, registerHref }: LoginFormProps) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const result = await loginAction(formData.email, formData.password, role);
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* Divider + Google Sign In (hidden for employer login) */}
            {!hideGoogleLogin && (
                <>
            ...
            <p className="text-center text-xs text-gray-600 mt-4">
                ...
                {registerHref ? (
                    <Link
                        href={registerHref}
                        onClick={onClose}
                        className="text-[#0f766d] font-semibold hover:text-[#0d5c55] transition-colors"
                    >
                        Create one
                    </Link>
                ) : (
